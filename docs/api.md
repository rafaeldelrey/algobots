...
| `angle` | Number | Current hull rotation angle (in radians) |
| `turret_angle` | Number | Current absolute turret angle (in radians) |
| `relativeTurretAngle` | Number | Current relative angle between hull and turret (in radians) |
...
| Method | Parameters | Returns | Description |
|--------|------------|---------|-------------|
| `turnTurret(targetAngleDegrees)` | targetAngleDegrees: Number | Boolean | Rotates the turret toward the specified angle in degrees. |
...
| Method | Parameters | Returns | Description |
|--------|------------|---------|-------------|
| `scan(relativeAngleDegrees, arcDegrees)` | relativeAngleDegrees: Number, arcDegrees: Number | Array | Scans for enemies in the specified cone. Returns an array of enemy objects if enemies are detected, or an empty array if none found or on cooldown. |
...
function runBotAI(botInfo, api, memory) {
    ...
    // Scan while moving
    api.turnTurret((botInfo.relativeTurretAngle * 180 / Math.PI + 5) % 360);
    ...
    // Basic scanning and shooting - using new direct scan method
...
function runBotAI(botInfo, api, memory) {
    ...
    switch (memory.state) {
        ...
        case "pursue":
            ...
            const turretDiff = Math.abs(api.normalizeAngle(botInfo.turret_angle * 180 / Math.PI - angleToTarget));
            if (turretDiff < 10) {
                api.fire();
                
                // Use overburn when firing
